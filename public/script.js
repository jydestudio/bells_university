...
map.on('load', () => {
    ...
    fetch('assets/shapefiles/upper_flow.json')
        ...
        .then(data => {
            ...
            // map.addLayer({
            //     'id': 'top_floor_transparent_layer',
            //     'type': 'fill-extrusion',
            //     'source': 'top_floor',
            //     'paint': {
            //         'fill-extrusion-color': ['get', 'color'],
            ...
            map.on('click', 'building-extrusion-layer2', (e) => {
                ...
                if (e.features.length > 0) {
                    ...
                    // Set the filter on the highlight layer to match the clicked feature's unique ID
            ...
            // C. Add a line layer for the outlines
            // map.addLayer({
            //     'id': 'building-line-layer2',
            //     'type': 'line',
            //     'source': 'building2',
            //     'paint': {
...
function addFilteredData(feature){
    console.log(feature);
    map.removeLayer("highlighted_office");
    
    
    if (feature.properties.baseheight == 9){
        map.removeLayer("ground_floor_plan");
        map.removeLayer("ground_doors_layer");
        map.removeLayer("top_floor_layer");
        // map.removeLayer("ground_floor_layer");
        // map.setPaintProperty('top_floor_layer', 'fill-extrusion-opacity', 0.1);
        ...
        map.setPaintProperty('ground_floor_layer', 'fill-extrusion-opacity', 0.1);
        map.setPaintProperty('top_floor_layer', 'fill-extrusion-opacity', 0.1);
        map.removeLayer("ground_floor_layer");
        map.removeLayer("top_floor_layer");
        map.removeLayer("top_floor_plan");
        map.removeLayer('seperator');
        map.removeLayer('top_doors_layer');
        ...
        map.addLayer({
            ...
            'paint': {
                'line-color': '#FF4C4C', // The specified black color
                'line-width': 8, // The specified line width
                'line-opacity': 1 // Full opacity (100%)
            }
        });
    ...
    if (feature.properties.baseheight == 9){
        pop = new maplibregl.Popup({ closeOnClick: true, closeButton: false, offset: [0, -80]})
        .setLngLat([longitude, latitude])
        .setHTML(`
